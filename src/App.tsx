    ...
    >
      ...
      <form onSubmit={handleJoin} className="space-y-6">
        <div className="space-y-4">
          ...
          <div>
            <label className="status-label mb-2 block ml-2">작업조 정보</label>
            <input 
              type="text"
              placeholder="작업조 입력 (예: 1조, 관리팀)"
              value={inputGroup}
  ...
  const PTTView = () => (
    ...
    >
      ...
      <header className="px-8 pt-10 pb-6 flex flex-col items-center border-b border-white/5 bg-industrial-card/50 backdrop-blur-xl">
        ...
        <div className="w-full bg-white/5 rounded-2xl p-4 flex items-center justify-between border border-white/10">
          <div className="flex flex-col">
            <span className="status-label text-[10px]">현재 작업조</span>
            <span className="text-2xl font-black text-white uppercase">{profile?.group}</span>
          </div>
          <div className="flex flex-col items-end">
            <span className="status-label text-[10px]">접속자</span>
            <span className="text-xl font-bold text-industrial-muted">{activeMembers.length + 1}명</span>
          </div>
        </div>
      </header>
  ...
  return (
    <div className="flex flex-col h-screen max-w-md mx-auto bg-industrial-bg overflow-hidden select-none">
      <AnimatePresence mode="wait">
        {view === 'AUTH' ? <AuthView key="auth" /> : <PTTView key="ptt" />}
      </AnimatePresence>
    </div>
  );
}
